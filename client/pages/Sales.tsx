...
export default function Sales() {
  ...
  const update = (i: number, patch: Partial<SaleItem>) =>
    setItems(items.map((it, idx) => (idx === i ? { ...it, ...patch } : it)));

  const subtotal = useMemo(
    () => items.reduce((s, i) => s + i.qty * i.price - (i.discount || 0), 0),
    [items],
  );
  ...
  const save = () => {
    ...
    if (!id) {
      setMessage("Insufficient stock for one or more items");
      return;
    }
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Create sales invoices with discount and tax
          </p>
        </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Stock will decrease when invoice is saved
          </CardDescription>
        </CardHeader>
        <CardContent>
          <Table>
                      ...
                      <Select
                        ...
                        onValueChange={(v) =>
                          update(idx, {
                            productId: v,
                            price:
                              products.find((p) => p.id === v)?.sellPrice || 0,
                          })
                        }
                      >
