    ...
    () =>
      filteredSales.reduce(
        (s, inv) =>
          s +
          inv.items.reduce(
            (a, i) => a + i.qty * i.price - (i.discount || 0),
            0,
          ) -
          (inv.discount || 0),
        0,
  ...
  const purchaseCost = useMemo(
    () =>
      filteredPurchases.reduce(
        (s, p) => s + p.items.reduce((a, i) => a + i.qty * i.cost, 0),
        0,
      ),
    [filteredPurchases],
  );
  const cogsApprox = useMemo(
    () =>
      filteredSales.reduce(
        (s, inv) =>
          s +
          inv.items.reduce((a, i) => {
            const p = products.find((pr) => pr.id === i.productId);
            return a + (p?.costPrice || 0) * i.qty;
          }, 0),
        0,
      ),
      ...
      <div className="grid md:grid-cols-2 gap-4">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Revenue and tax</CardDescription>
          </CardHeader>
          <CardContent>
          ...
          <Table>
            ...
            <TableBody>
              {filteredSales.map((s) => {
                ...
                return (
                  <TableRow key={s.id}>
                    <TableCell>{s.date.slice(0, 10)}</TableCell>
