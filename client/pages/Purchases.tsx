...
export default function Purchases() {
  ...
  const [supplierId, setSupplierId] = useState<string|undefined>(suppliers[0]?.id);
  const [items, setItems] = useState<PurchaseItem[]>([]);

  const addRow = () => setItems([...items, { productId: products[0]?.id!, qty: 1, cost: products[0]?.costPrice || 0 }]);
  const update = (i:number, patch:Partial<PurchaseItem>) => setItems(items.map((it,idx)=> idx===i? { ...it, ...patch }: it));
  const total = useMemo(()=> items.reduce((s,i)=> s + i.qty * i.cost, 0), [items]);
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Create purchase orders and receive stock</p>
        </div>
          ...
          <Table>
            ...
            <TableBody>
              {items.map((it, idx)=> (
                <TableRow key={idx}>
                  <TableCell>
                    <Select value={it.productId} onValueChange={(v)=>update(idx,{ productId:v, cost: products.find(p=>p.id===v)?.costPrice || 0 })}>
                  ...
                  <TableCell className="text-right"><Input type="number" step="0.01" className="text-right" value={it.cost} onChange={(e)=>update(idx,{ cost: parseFloat(e.target.value||"0") })} /></TableCell>
                  <TableCell className="text-right">{(it.qty*it.cost).toFixed(2)}</TableCell>
